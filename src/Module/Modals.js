import { AbstractModule, Ajax, Events, App } from 'hamtaraw';
import $ from 'jquery';
...
class ModalsModuleClass extends AbstractModule {
    ...
    show(ctrl, oData = {}) {
        ...
        const _this = this;
        const ModalElement = document.querySelector(`.hamtaraw-modal[data-ctrl="${ctrl}"]`);
        ...
        if (ModalElement) {
            ...
            let BackdropDiv = document.querySelector('#HamtarawModalBackdrop div');
            if (BackdropDiv) {
                BackdropDiv.remove();
            }
            ...
            $('body').append(`
                <div id="HamtarawModalBackdrop" class="modal-backdrop fade show">
                  <div>Loading...</div>
                </div>
           `);

            $('#HamtarawModalBackdrop').animate({opacity: 0.5}, 0, function () {
                return Ajax.loadModal(ctrl, oData).then((AxiosResponse) => {
                    if (AxiosResponse['data']['success']) {
                        //$('#HamtarawModalBackdrop').remove();
